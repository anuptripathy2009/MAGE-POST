...
def edit_post(post_id):
    ...
    if request.method == 'POST':
        ...
        if not title or not caption:
            flash('Title and caption are required.', 'error')
            return redirect(url_for('edit_post', post_id=post.id))

        if image:
            # Update the post's image if a new image is provided
            os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)
            image_filename = datetime.datetime.now().strftime('%Y%m%d%H%M%S') + '_' + image.filename
            image.save(os.path.join(app.config['UPLOAD_FOLDER'], image_filename))
    ...
    return render_template('edit_post.html', post=post, username=user.username)
...
def delete_post(post_id):
    ...
    # Delete the post's image from the UPLOAD_FOLDER
